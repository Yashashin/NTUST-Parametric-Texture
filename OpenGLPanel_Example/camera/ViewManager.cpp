...
vec3 ViewManager::GetWorldEyePosition() {
	...
	return vec3(wordEyePosition.x, wordEyePosition.y, wordEyePosition.z);
}
...
vec3 ViewManager::GetWorldViewVector() {
	...
	return vec3(wordLookVector.x, wordLookVector.y, wordLookVector.z);
}
...
void ViewManager::mousePressEvent(int button, int x, int y)
{
    if( button == GLUT_LEFT_BUTTON)
	{
		...
		vec3 result = (invrtRot * vec4(0, 1, 0, 0));
		rotateYAxis = result;
		result = (invrtRot * vec4(1, 0, 0, 0));
		rotateXAxis = result;
    } 
...
void ViewManager::mouseMoveEvent(int x,int y)
{
	...
	else if(midDown)
	{
		...
		float factor = diff.y / (float)w_height;
		vec3 diffUp = vec3(up.x * factor, up.y * factor, up.z * factor);
		factor = diff.x / (float)w_width;
		vec3 diffRight = vec3(right.x * factor, right.y * factor, right.z * factor);
...
void ViewManager::SetRotation(float theta, float phi)
{
    ...
    rotationMatrix = rotate(rotationMatrix, glm::radians(theta), vec3(0, 1, 0));
    rotationMatrix = rotate(rotationMatrix, glm::radians(phi), vec3(1, 0, 0));
}
